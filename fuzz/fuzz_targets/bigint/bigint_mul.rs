...
impl BigIntConfig {
    pub fn create_sub_script(&self) -> Vec<u8> {
        ...
        bytes.extend_from_slice(match_bigint_type!(self.bigint_type, mul).compile().as_bytes()); // c * b
        bytes.extend_from_slice(match_bigint_type!(self.bigint_type, mul).compile().as_bytes()); // c * b * a

        let a = BigUint::from_slice(self.a.as_ref());
        let b = BigUint::from_slice(self.b.as_ref());
        let mut c = BigUint::from_slice(self.c.as_ref());

        let modulo = BigUint::one().shl(self.bigint_type.n_bits());
        c = (&c * &b).rem(&modulo);
        c = (&c * &a).rem(&modulo);

        let push_answer = match_bigint_type!(self.bigint_type, push_u32_le, &c.to_u32_digits());
...
fuzz_target!(|message: BigIntConfig| {
    let mut bytes = message.create_sub_script();
